...
NOW=$(date +%s)
JSEQ="${JSEQ_OVERRIDE:-DEV-SMOKE-SKEW-001}"

PAYLOAD=$(cat <<JSON
{"journalSeq":"$JSEQ","amount":1500000,"custCode":"$CUST_CODE","bankCode":"390","brivaNo":"$BRIVA_NO","paidAt":"$(date -Ins)"}
JSON
)

if [[ "out_of_skew" == "out_of_skew" ]]; then
  # mundurkan 3600 detik (1 jam) biar di-drop kalau skew ketat
  NOW=$((NOW - 3600))
fi

if [[ "out_of_skew" == "bad_sig" ]]; then

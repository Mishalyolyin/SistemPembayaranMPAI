{{-- resources/views/auth/reguler/passwords/email.blade.php --}}
<!DOCTYPE html>
<html lang="id">
<head>
  <meta charset="UTF-8" />
  <title>Lupa Password – Mahasiswa Reguler</title>
  ...
  <style>
    :root{
      ...
      --accentSoft: rgba(59,130,246,.24);
    }
    html,body{height:100%}
    body{
      min-height:100vh;
      background:
        radial-gradient(950px 580px at 15% 0%, rgba(29,78,216,.22), transparent 55%) no-repeat,
        radial-gradient(950px 580px at 85% 12%, rgba(20,184,166,.18), transparent 55%) no-repeat,
    ...
    .form-control:focus{
      ...
      box-shadow: 0 0 0 .2rem var(--accentSoft)
    }
    ...
    .btn-primary:disabled{ opacity:.95 }
...
<body>
    ...
    <div class="glass-card" role="region" aria-labelledby="page-title">
      <div class="card-head">
        <h1 id="page-title" class="headline">
          <i class="bi bi-envelope-check me-2"></i>Lupa Password (Mahasiswa Reguler)
        </h1>
      ...
      <div class="content">
        ...
        <form method="POST" action="{{ route('reguler.password.email') }}" class="needs-validation" novalidate>
          @csrf
          <div class="mb-3">
        ...
        <div class="footer-note">
          <i class="bi bi-lock"></i> Mode DEV: email dikirim ke Mailtrap / log file.
        </div>
      </div>
    </div>
  </div>
  ...
  <script>
    // Loading state ketika form valid

...
<head>
  ...
  <style>
    * { box-sizing: border-box; }
    @page { margin: 28px; }
    body { font-family: DejaVu Sans, Arial, sans-serif; margin: 0; font-size: 12px; color: #111; }
    ...
    /* (Opsional) Gambar TTD/Cap dari storage/kwitansi/ttd.png */
    .sign img { max-width: 140px; height: auto; display:block; margin: 24px auto 6px; }
    .stamp-line { margin-top: 60px; }
  </style>
</head>
<body>
@php
  ...
  // ===== tanggal bayar terbaik (urut: verified_at -> tanggal_bayar -> uploaded_at -> updated_at -> created_at)
  ...
  $fmtRupiah = fn ($v) => number_format((int)$v, 0, ',', '.');

  // ===== label semester/TA fallback
  $semLabel = $invoice->semester ?? ($mahasiswa->semester_awal ?? null);
  $taLabel  = $invoice->tahun_akademik ?? ($mahasiswa->tahun_akademik ?? null);

  // ===== (opsional) ambil gambar ttd dari public/storage/kwitansi/ttd.png
  $ttdPath = public_path('storage/kwitansi/ttd.png');
  $ttdData = (is_file($ttdPath))
...
@endphp
  ...
  <table class="meta">
    ...
    <tr>
      <th>Semester / TA</th>
      <td>{{ $semLabel ? ucfirst($semLabel) : '—' }}{{ $taLabel ? ' / '.$taLabel : '' }}</td>
    </tr>
    <tr>
      ...
      <td>
        ...
        @if(!empty($invoice->angsuran_ke))
          &nbsp;<span class="muted">(Angsuran ke-{{ $invoice->angsuran_ke }})</span>
        @endif
      </td>
    </tr>
    <tr>
    ...
    <tr>
      ...
      <td>
        @if($tglBayar)
          {{ Carbon::parse($tglBayar)->translatedFormat('d F Y') }}
        @else
          —
        @endif
      </td>
  ...
  <div class="foot">
    ...
    <div class="sign">
      ...
      @if($ttdData)
        <img src="{{ $ttdData }}" alt="TTD">
      @else
        <div class="stamp-line">__________________________</div>
      @endif
    </div>
  </div>
</body>
